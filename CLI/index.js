...
function b64toBuffer(base64String) {
...
function b2p(buffer) {
    ...
    readFile(inputBufferPath, "utf-8", (err, data) => {
        if (err) {
            console.log(chalk.bgRed(err.message));
        }
        else {
            // creating an image with the parsed buffer code
            writeFile(outputImagePath, b64toBuffer(data), (err) => {
                if (err) {
                    console.error(err);
                }
                else {
...
function p2b(paths) {
    ...
    readFile(inputImagePath, (err, data) => {
        if (err) {
            console.log(chalk.bgRed(err.message));
        }
        else {
            ...
            writeFile(outputBufferPath, parsedData, (err) => {
                if (err) {
                    ...
                    console.log(chalk.green("Done!"));
                    console.log("The buffer is encoded in Base64, Here's the file path:", chalk.blue(outputBufferPath));
                }
            });
        }
    });
...
async function getPathOrBuffer(option) {
    if (option == "P2B - Picture to Buffer") {
        //if P2B:
        const answers = await inquirer.prompt({
            name: "getPath",
            type: "input",
            message: "Place your image file in the Desktop dir and add the file name here: ",
        });
    ...
    if (option == "B2P - Buffer to Picture") {
        const answers = await inquirer.prompt({
            name: "getBuff",
            type: "input",
            message: "Place the buffer.txt file in the Desktop dir then add the file name here : ",
        });
